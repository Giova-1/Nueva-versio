...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.i18n) {
        ...
        document.addEventListener('i18n:ready', () => {
            console.log('i18n ready, initializing components');
...
function initLanguageSelector() {
    // Asegurarse de que i18n esté disponible
    if (!window.i18n) {
        console.error('i18n no está disponible');
        return;
    }
    ...
    document.addEventListener('languageChanged', (event) => {
        ...
        if (window.languageSelector) {
            window.languageSelector.updateState();
        }
    });

    // Escuchar el evento i18n:ready
    document.addEventListener('i18n:ready', () => {
        // Cargar el idioma guardado o usar el predeterminado
        const savedLang = localStorage.getItem('preferred_language');
        
        if (savedLang && window.i18n.config.availableLanguages.includes(savedLang)) {
            window.i18n.changeLanguage(savedLang);
        } else {
            // Usar el idioma del navegador o español por defecto
            const browserLang = navigator.language.split('-')[0];
            const defaultLang = window.i18n.config.availableLanguages.includes(browserLang) 
                ? browserLang 
                : 'es';
            window.i18n.changeLanguage(defaultLang);
        }
    });
}
